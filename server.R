...
winning <- function(team, opp) {
  ...
  tBPI      <- as.numeric(tESPN[grep(Sys.Date(), tESPN$DATE), 4])
  tBPI      <- if (length(tBPI) == 0L) 0 else as.numeric(tESPN[grep(Sys.Date(), tESPN$DATE), 4])
  oESPN     <- dfTeams[dfTeams$Yahoo==opp, 1];  oESPN <- recentPerf[recentPerf$TEAM==oESPN, ] 
  oBPI      <- as.numeric(oESPN[grep(Sys.Date(), oESPN$DATE), 4])
  oBPI      <- if (length(oBPI) == 0L) 0 else as.numeric(oESPN[grep(Sys.Date(), oESPN$DATE), 4])
  
  # Get corresponding recent performance data
  
  pretRP    <- dfTeams[dfTeams$Yahoo==team, 1]; pretRP <- recentPerf[recentPerf$TEAM==pretRP, ]
  tRP       <- as.numeric(pretRP[grep(Sys.Date(), pretRP), 5])
  tRP       <- if (length(tRP) == 0L) 0 else as.numeric(pretRP[grep(Sys.Date(), pretRP), 5])
  preoRP    <- dfTeams[dfTeams$Yahoo==opp, 1]; preoRP <- recentPerf[recentPerf$TEAM==preoRP, ]
  oRP       <- as.numeric(preoRP[grep(Sys.Date(), preoRP), 5])
  oRP       <- if (length(oRP) == 0L) 0 else as.numeric(preoRP[grep(Sys.Date(), preoRP), 5]) 
